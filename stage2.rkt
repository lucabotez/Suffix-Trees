...
(define (get-ch-words words ch)
  (filter (compose not null?) (map (λ (word) (if (equal? (car word) ch) word null))
                                   (filter (compose not null?) words))))
...
(define (ast-func suffixes)
  (cons (list (caar suffixes))
        (map (λ (word) (cdr word)) suffixes)))
...
(define (suffixes->st labeling-func suffixes alphabet)
  (define sorted-words (map (λ (letter) (get-ch-words suffixes letter)) alphabet))
  (define sorted-trees (map labeling-func (filter (compose not null?) sorted-words)))
 
  (cond
    ((null? sorted-trees) null)
    (else (map (λ (subtree) (append (list (get-branch-label subtree))
...
(define (text->st text)
  (define alphabet (sort (remove-duplicates (append text '(#\$))) char<?))
  (λ (labeling-func) (suffixes->st labeling-func (get-suffixes (append text '(#\$))) alphabet)))
