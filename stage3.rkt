...
(define (longest-common-substring text1 text2)
  ...
  ;; functie ce intoarce cel mai lung string din 2 stringuri
  (define (longest-string string1 string2)
    (cond
      ((< (length string1) (length string2)) string2)
      (else string1)))

  ;; functie ce se ocupa cu pasul 2
  (define (helper stree suffix)
    (cond
      ((null? stree) null)
      (else (let ((res (match-pattern-with-label stree suffix)))
              (cond
                ((equal? res #t) suffix)
                ((car res) (append (car res) (helper (caddr res) (cadr res))))
                (else (cadr res)))))))

  (let ((stree1 (text->cst text1)))
    (let iter ((suffixes (get-suffixes text2)) (result null))
      (if (null? suffixes)
          result
          (iter (cdr suffixes) (longest-string result (helper stree1 (car suffixes))))))))
...
(define (repeated-substring-of-given-length text len)
  ...
  (define (dfs stree res)
    (cond
      ((null? stree) #f)
      ((equal? (length res) len) res)
      (else (let ((branch (first-branch stree)))
              (or (dfs (get-branch-subtree branch) (append res (get-branch-label branch)))
                  (dfs (other-branches stree) res))))))
